...
1. 解析字幕（**MVP 阶段只支持 `.srt` 格式**，使用 `pysrt` 库解析）；
2. 按时间轴切分剧情片段；
3. 判断每个片段是否为高光；
...
```text
- 支持格式：.srt（SubRip Text）
- 不支持格式：.ass / .vtt / .ssa（MVP 阶段不处理）
- 后端上传字幕时应校验文件后缀为 .srt，否则返回 400 错误
- 测试数据中的示例字幕文件统一使用 .srt 格式提供
```
...
| 用户行为采集 | 接收用户点击、忽略、长按等互动日志 |
| 数据统计分析 | 计算点击率、高光类型分布、热门按钮等指标 |
...
| subtitle_url | VARCHAR | 字幕地址，MVP 阶段只接受 .srt 文件 |
...
### 11.4 interaction_template：互动模板表

按高光类型预设默认按钮、特效和展示配置，供 interaction_config 复用。

```sql
CREATE TABLE interaction_template (
  id VARCHAR(64) PRIMARY KEY,
  highlight_type VARCHAR(50) NOT NULL UNIQUE,
...
|---|---|
| id | 模板 ID |
| highlight_type | 对应高光类型，唯一索引，一种类型只有一个默认模板 |
| interaction_type | 互动组件类型 |
| template_json | 默认按钮、特效、展示配置，结构同 config_json |

初始化数据（系统启动时写入）：

| highlight_type | 按钮 | 特效 |
|---|---|---|
| conflict | 气死我了 / 替她反击 / 支持女主 | anger_bar |
| reversal | 我惊了 / 反转了 / 太爽了 | screen_flash |
| sweet | 磕到了 / 太甜了 / 锁死 | heart_rain |
| satisfying | 爽 / 打脸成功 / 终于来了 | boom_effect |
| suspense | 快更 / 别卡这里 / 我猜到了 | countdown |

### 11.5 interaction_config：互动配置表

每个高光点一条配置记录。优先使用 config_json 中的自定义内容；若 config_json 为空，则 fallback 到 template_id 对应的模板。
...
CREATE TABLE interaction_config (
  ...
  FOREIGN KEY (template_id) REFERENCES interaction_template(id)
);
```

| 字段 | 说明 |
|---|---|
| template_id | 关联模板 ID，AI 识别后自动按 highlight_type 匹配 |
| config_json | 运营人员手动覆盖的配置，为 NULL 时使用模板内容 |

后端渲染播放数据时的优先级：

```text
config_json 不为空 → 使用 config_json
config_json 为空 → 查询 template_id 对应的 template_json
两者均为空 → 过滤该高光点，不下发到播放端
```
...
### 11.6 user_interaction_log：用户互动日志表
...
每次 `POST /api/interactions` 写入 `user_interaction_log` 后，立即执行：

```sql
INSERT INTO highlight_stats (id, highlight_id, click_count, ignore_count)
VALUES (uuid(), :highlight_id, 0, 0)
ON DUPLICATE KEY UPDATE
  click_count = click_count + IF(:action_type = 'click', 1, 0),
  ignore_count = ignore_count + IF(:action_type = 'ignore', 1, 0),
  top_action = :action_value,
  updated_at = NOW();
```

MVP 阶段并发量低，同步写入完全够用。若后续并发量大，可升级为异步汇总任务，无需改动表结构。
...
```json
{
  "episode_id": "ep_001",
  "title": "第 1 集：身份反转",
  "video_url": "https://example.com/videos/ep001.mp4",
  "duration": 420,
  "highlights": [
    {
      "highlight_id": "hl_001",
...
```json
{
  ...
  "message": "AI 高光识别已启动，请轮询 /analyze-status 获取结果"
}
```
...
```text
使用 FastAPI BackgroundTasks 将 AI 识别任务放入后台执行，接口在更新
analyze_status = processing 后立即返回，前端通过轮询
GET /api/episodes/{episode_id}/analyze-status 获取最终结果。
```
...
3. 如果已经 success 且 force_reanalyze=false，则拒绝重复分析；
4. 如果 analyze_status = processing，则拒绝重复触发；
5. 更新 episode.analyze_status = processing；
6. 将 AI 识别任务注册到 BackgroundTasks；
7. 立即返回 processing 状态。

【后台任务，异步执行】
1. 调用 AI 高光识别服务；
2. 校验 AI 返回 JSON 是否合法；
3. 过滤低置信度高光点；
4. 写入 highlight_event 表；
5. 为每个高光点生成默认 interaction_config（优先匹配 interaction_template）；
6. 更新 episode.analyze_status = success；
7. 如果任何步骤异常，更新 episode.analyze_status = failed 并记录错误日志。
```
...
async def trigger_analyze(
    ...
    if episode.analyze_status == "processing":
        raise HTTPException(409, "分析任务正在执行中")
    if episode.analyze_status == "success" and not body.force_reanalyze:
        raise HTTPException(409, "已分析完成，如需重新分析请传 force_reanalyze=true")
    update_analyze_status(db, episode_id, "processing")
    background_tasks.add_task(run_highlight_analysis, episode_id)
    return {"success": True, "data": {"episode_id": episode_id, "analyze_status": "processing"}}
```
...
4. action_type 必须属于 click/ignore/long_press；
...
```text
1. 前端发送后设置 flag，防止重复提交；
2. 后端生成 idempotency_key；
3. 基于 user_id、episode_id、highlight_id、action_type、watch_time 做短时间去重。
```
